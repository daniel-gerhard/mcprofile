...
\title{An introduction to the mcprofile package}
\date{\today}
...
%\VignetteIndexEntry{Using mcprofile} \\
...
The package mcprofile contains functions for simultaneous inference based on a set of profile statistics, controlling the family-wise error rate (FWER) at a specified level. Focus is set to testing general linear hypotheses in generalized linear models and provide a set of compatible simultaneous confidence intervals. The multiple testing framework of \citet{Hothorn2008} is adopted to signed likelihood root and modified likelihood root statistics to improve the performance especially at small sample sizes.
...
Several test statistics can be used to test the set of general linear hypotheses \citep{Brazzale2008}.
...
\subsubsection{Signed likelihood root}
...
Based on the profiles of the likelihood root, confidence intervals are calculated with the argument \texttt{adjust="none"} to control the per comparison error rate with a separate type-I-error for each comparison.
...
The functionality of the package multcomp with the \texttt{contrMat} function can be used to use several pre-defined structures for the contrast matrix.
...
The calculated profiles can simply be plotted by calling plot. The ggplot2 function \texttt{ylim} is used to restrict the y-axis scale to a range $[-5,5]$
...
The confidence intervals are shown on the scale of the linear predictor. The mcprofile package contains an \texttt{exp} method to transform the scale. Further ggplot2 functions can be used to change the plot layout.
...
\subsection{Modifying the profile support grid}
...
For example, the profiles can be calculated for a range of 25 support values between $[-3, 5]$.
...
\subsection{Simultaneous confidence intervals and multiple testing}
...
Compatible tests are available with the \texttt{summary} method. Here, further multiple testing adjustment is available through the function \texttt{p.adjust}. A single test margin can be specified to define the Null-hypotheses together with a test direction.
...
\section{Wald-type statistics}

Instead of the likelihood root, the Wald-type statistic presented in Section \ref{wald} can be used to construct a profile. The function \texttt{wald} directly transforms the profile object.
...
The simultaneous confidence intervals are exactly the same as the ones obtained with the function \texttt{glht} in package multcomp.
<<multcomp>>=
confint(glht(m2, cm2))
confint(w2)
@
...
\section{Modified likelihood root}

At small sample sizes, the likelihood root statistic can be exchanged with the modified likelihood root, presented in Section \ref{modifiedlikelihoodroot}, to improve the performance of the confidence intervals. 
...
\section{Simultaneous confidence intervals for predictions in logistic regression models}
...
\bibitem[Brazzale and Davison(2008)]{Brazzale2008}
Alessandra~R. Brazzale and Anthony~C. Davison.
\newblock Accurate parametric inference for small samples.
\newblock \emph{Statistical Science}, 23\penalty0 (4):\penalty0 465--484, 2008.
\newblock \doi{10.1214/08-STS273}.
...
\newblock \emph{Biometrical Journal}, 50\penalty0 (3):\penalty0 346--363, 2008.
\newblock \doi{10.1002/bimj.200810425}.
